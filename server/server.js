...
app.get("/article", async (req, res) => {
  const query =
    "SELECT * FROM article , `user` WHERE article.user_seq = user.seq";
...
app.get("/getarticle", async (req, res) => {
  const { seq } = req.query;
  const query = `SELECT * FROM article , user WHERE article.user_seq = user.seq AND article.seq = ${seq}`;
  const article = await 디비실행(query);
  res.send(article[0]);
});

app.post("/reply", async (req, res) => {
  const { reply, seq } = req.body;
  ...
  console.log(reply);
  console.log(seq);
  const query = `INSERT INTO reply (body,user_seq,article_seq) VALUES ('${reply}',${loginUser.seq},${seq})`;
  await 디비실행(query);
  console.log(query);
  res.send(result);
});
